...
import { Menu, X, ChevronDown, Info, Camera } from 'lucide-react';
              ...
              <AnimatePresence>
                {megaOpen && (
                  <motion.div
                    ...
                    className="absolute top-full left-1/2 -transtone-x-1/2 mt-2 w-[560px] glass rounded-2xl shadow-2xl p-4 grid grid-cols-2 gap-2"
                  >
              ...
              <AnimatePresence>
                {aboutOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 10 }}
                    transition={{ duration: 0.2 }}
                    className="absolute top-full right-0 mt-2 w-72 glass rounded-2xl shadow-2xl p-2"
                  >
                    <Link to="/about" className="flex items-start gap-3 p-3 rounded-xl hover:bg-primary/5 transition-colors">
                      <span className="w-10 h-10 shrink-0 rounded-xl bg-primary/10 text-primary flex items-center justify-center">
                        <Info size={18} />
                      </span>
                      <span>
                        <span className="block text-sm font-semibold text-text-primary">About Us</span>
                        <span className="block text-xs text-text-secondary mt-0.5">Who we are &amp; what we stand for</span>
                      </span>
                    </Link>
                    <Link to={`${BASE}/moments`} className="flex items-start gap-3 p-3 rounded-xl hover:bg-primary/5 transition-colors">
                      <span className="w-10 h-10 shrink-0 rounded-xl bg-secondary/10 text-secondary flex items-center justify-center">
                        <Camera size={18} />
                      </span>
                      <span>
                        <span className="block text-sm font-semibold text-text-primary">Life at Globe Creater</span>
                        <span className="block text-xs text-text-secondary mt-0.5">Photos &amp; events from our journey</span>
                      </span>
                    </Link>
                  </motion.div>
                )}
          ...
          >
            ...
            >
              ...
              {[
                ...
                ['About Us', '/about'],
                ['Life at Globe Creater', `${BASE}/moments`],
              ].map(([label, href]) => (
