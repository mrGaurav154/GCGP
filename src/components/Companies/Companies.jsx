...
// Resolves a per-company accent color used to tint the whole card on hover.
function getAccentColor(name) {
  if (name === 'Google') return googleLetterColors[0];
  if (name === 'Microsoft') return '#00A4EF';
  return brandColors[name]?.text ?? '#0D9488';
}
  ...
  return (
    ...
    >
      {list.map((name, i) => (
        <div
          key={`${name}-${i}`}
          className="accent-card flex items-center justify-center px-8 py-5 rounded-2xl border border-stone-100 bg-white min-w-[160px]"
          style={{ '--accent': getAccentColor(name) }}
        >
  ...
  return (
    <section className="section-pad !pt-10 sm:!pt-16">
      <div className="container-px">
        ...
        <div className="relative overflow-hidden mx-auto max-w-6xl marquee-group">
